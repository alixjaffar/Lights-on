...
/**
 * @Author: Ali Jaffar
 * @Description: This is the hall screen. It is where the player will be able to move around and interact with the world.
 */

public class HallScreen implements Screen {
 ...
 public void camera() {
    ...
    public void show() {
         ...
         // get the tmx file
         TiledMap tiledMap = new TmxMapLoader().load("Hall_Bloody.tmx");
    ...
    public void render(float delta) {    
        ...
        player.draw(batch);
            ...
            batch.begin();
               ...
               batch.begin();
               font.draw (batch, "Press C to go down to the lobby.", 550, 150);
               font.setColor(Color.WHITE);
               if (Gdx.input.isKeyPressed(Input.Keys.C)) {
                    gameReference.setScreen(new Lobby(gameReference));
               }
               batch.end();

               batch.begin();
               font.draw (batch, "You need a key to enter the locked room.", 400, 100);
               font.setColor(Color.WHITE);
               if (Apartment1.keyCollected == true) {
                    if (Gdx.input.isKeyPressed(Input.Keys.F)) {
                         gameReference.setScreen(new WaterScreen(gameReference));
                    }
                    else {
                         font.draw (batch, "Press F to enter the room.", 400, 150);
                    }
               }
               batch.end();
